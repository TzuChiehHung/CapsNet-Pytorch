...
def demo(model, test_loader, args):
    import numpy as np
    import cv2
    ...
    for x, y in test_loader:
        ...
        img = x.data.cpu().numpy()[0][0]

        cv2.imshow('Image', img)
        print('predict: %i, ground truth: %i' % (y_pred.item(), y_true.item()))
...
if __name__ == "__main__":
    ...
    if args.action == 'train':
        ...
        if args.batch_size > 1:
            print('Set batch size to 1 for demo.')
        if args.weights is None:
            print('No weights are provided. Will test using random initialized weights.')
        demo(model=model, test_loader=test_loader, args=args)
